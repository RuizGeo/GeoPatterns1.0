...
def model_kNN(bar,path_train,dataset_path,\
                               path_val,start_k,\
        ...
        #Write 
        f_txt.write('Dataset;k;metric;kappa'+'\n')
        ...
        for i_n, seg in enumerate(dataset_names):                
            ...
            for k_v in range(int(start_k),int(end_k)+int(step_k),int(step_k)):
                for p_v in metric:
                    #criar modelo Random Forest
                    clf = neighbors.KNeighborsClassifier( n_neighbors=k_v,p=p_v,weights=weigth_feature)
                    #Ajustar modelo
                    modelkNN = clf.fit(dfjt[features].values, dfjt[field_class_train])
                    #Classificar
                    clas = modelkNN.predict(dfjv[features].values)
                    #Calculate kappa
                    kappa = metrics.cohen_kappa_score(dfjv[field_class_val],clas)                   
                    #Calculate PC
                    #pc,qd,qa,matrix=pontius2011(dfjv[field_class_val],clas)
                    #print (pc,qd,qa)
                    f_txt.write(seg+';'+str(k_v)+';'+ str(p_v)+';'+str(round(kappa,4))+'\n') 
                    #Avaliar a acuracia
        ...
        if bool(state_applyclass) :
            ...
            #criar modelo KNN
            clf = neighbors.KNeighborsClassifier( n_neighbors=best_parameters['k'],p=best_parameters['metric'],weights=weigth_feature)
            #Ajustar modelo
